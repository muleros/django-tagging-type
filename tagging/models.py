...
class TagManager(models.Manager):
    def update_tags(self, obj, tag_names):
        ...
        for tag_name in updated_tag_names:
            if tag_name not in current_tag_names:
                ...
                t = TaggedItem._default_manager.create(tag=tag, object=obj)
    ...
    def _get_usage(self, model, counts=False, min_count=None,
        ...
        cursor.execute(query %
                       (extra_joins, extra_criteria, min_count_sql), params)
        tags = []
...
class MultiTagManager(models.Manager):
    def update_tags(self, obj, kind, tag_names):
        """
        Update tags associated with an object.
        """
        ctype = ContentType.objects.get_for_model(obj)
        ...
        tags_for_removal = [tag for tag in current_tags
                            ...
                            and tag.kind == kind]
        if len(tags_for_removal):
            TaggedItemKind._default_manager.filter(
                content_type__pk=ctype.pk,
                object_id=obj.pk,
                tag__in=tags_for_removal).delete()
        # Add new tags
        ...
        for tag_name in updated_tag_names:
            if tag_name not in current_tag_names:
                ...
                if created:
                    tag.save()
                TaggedItemKind._default_manager.create(tag=tag,
                                                       object=obj, )
    ...
    def add_tag(self, obj, tag_name):
        ...
        tag, created = self.get_or_create(name=tag_name)
        ctype = ContentType.objects.get_for_model(obj)
        TaggedItemKind._default_manager.get_or_create(
            tag=tag, content_type=ctype, object_id=obj.pk)
    ...
    def related_for_model(self, tags, model, counts=False, min_count=None):
        ...
        tags = get_tag_list(tags)
        tag_count = len(tags)
        tagged_item_table = qn(TaggedItemKind._meta.db_table)
        query = """
        SELECT %(tag)s.id, %(tag)s.name%(count_sql)s
        FROM %(tagged_item)s INNER JOIN %(tag)s ON %(tagged_item)s.tag_id = %(tag)s.id
        WHERE %(tagged_item)s.content_type_id = %(content_type_id)s
...
class TagKind(models.Model):
    ...
    name = models.CharField(_('name'), max_length=50, unique=True,
                            db_index=True)
    kind = models.CharField(_('kind'), max_length=50, db_index=True)

    objects = MultiTagManager()
...
class TaggedItemKind(models.Model):
    """
    Holds the relationship between a tag and the item being tagged.
    """
    tag = models.ForeignKey(TagKind, verbose_name=_('tag'),
                            related_name='items')
    content_type = models.ForeignKey(ContentType,
                                     verbose_name=_('content type'))
    object_id = models.PositiveIntegerField(_('object id'), db_index=True)
