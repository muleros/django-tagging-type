...
def do_tags_for_model(parser, token):
    ...
    if len_bits not in (4, 6):
        raise TemplateSyntaxError(
            _('%s tag requires either three or five arguments') %
            (bits[0], ))
    if bits[2] != 'as':
        raise TemplateSyntaxError(
...
def do_tag_cloud_for_model(parser, token):
    ...
    Examples::
       ...
       {% tag_cloud_for_model products.Widget as widget_tags with steps=9
       min_count=3 distribution=log %}
    """
    bits = token.contents.split()
    len_bits = len(bits)
    if len_bits != 4 and len_bits not in range(6, 9):
        raise TemplateSyntaxError(
            _('%s tag requires either three or between five and seven '
              'arguments') % (bits[0], ))
    if bits[2] != 'as':
        raise TemplateSyntaxError(
            ...
            try:
                ...
                if name == 'steps' or name == 'min_count':
                    try:
                        ...
                        raise TemplateSyntaxError(
                            _("%(tag)s tag's '%(option)s' option was not a "
                              "valid integer: '%(value)s'") %
                            {
                                'tag': bits[0],
                                'option': name,
                                'value': value,
...
def do_tagged_objects(parser, token):
    ...
    if len(bits) != 6:
        raise TemplateSyntaxError(
            _('%s tag requires exactly five arguments') %
            (bits[0], ))
    if bits[2] != 'in':
        raise TemplateSyntaxError(
            _("second argument to %s tag must be 'in'") %
            (bits[0], ))
    if bits[4] != 'as':
        raise TemplateSyntaxError(
            _("fourth argument to %s tag must be 'as'") %
            (bits[0], ))
